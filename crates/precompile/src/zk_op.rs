...
/// Enum representing the ZKVM operations that can be performed
...
pub trait ZkvmOperator: Send + Sync + 'static {
    fn bn128_run_add(&self, input: &[u8]) -> Result<[u8; 64], PrecompileError>;
    fn bn128_run_mul(&self, input: &[u8]) -> Result<[u8; 64], PrecompileError>;
    fn bn128_run_pairing(&self, input: &[u8]) -> Result<bool, PrecompileError>;
    fn blake2_run(&self, input: &[u8]) -> Result<[u8; 64], PrecompileError>;
    fn sha256_run(&self, input: &[u8]) -> Result<[u8; 32], PrecompileError>;
    fn ripemd160_run(&self, input: &[u8]) -> Result<[u8; 32], PrecompileError>;
    fn modexp_run(
        &self,
        base: &[u8],
        exp: &[u8],
